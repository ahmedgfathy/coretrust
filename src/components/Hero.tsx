        ...
        <div className="w-full px-5 sm:px-8 lg:px-20 pt-28 sm:pt-32 lg:pt-40 pb-32 sm:pb-0">
          <div className="max-w-5xl mx-auto lg:mx-0">
            ...
            <h1 className="font-heading text-3xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-bold text-white mb-4 sm:mb-6 leading-tight">
              {t(content.hero.titleEn?.split(',')[0] || 'Innovating Spaces', content.hero.titleAr?.split('،')[0] || 'ابتكار المساحات')} <span className="gold-shimmer block mt-1 sm:mt-2">{t(content.hero.titleEn?.split(',')[1]?.trim() || 'Building Futures', content.hero.titleAr?.split('،')[1]?.trim() || 'بناء المستقبل')}</span>
            </h1>
            ...
            <div className="grid grid-cols-2 sm:flex sm:items-center sm:space-x-8 lg:space-x-16 mt-10 sm:mt-16 gap-6 sm:gap-0">
              <div className="text-center sm:text-left">
                <span className="font-heading text-3xl sm:text-4xl lg:text-5xl font-bold text-[#d4a017]">{content.stats.value1 || '90+'}</span>
              ...
              <div className="text-center sm:text-left">
                <span className="font-heading text-3xl sm:text-4xl lg:text-5xl font-bold text-[#d4a017]">{content.stats.value2 || '20+'}</span>
                <p className="text-gray-400 text-[10px] sm:text-xs md:text-sm mt-1 sm:mt-2 uppercase tracking-wider">
              ...
              <div className="hidden sm:block w-px h-10 lg:h-14 bg-[#d4a017]/30"></div>
              <div className="text-center sm:text-left">
                <span className="font-heading text-3xl sm:text-4xl lg:text-5xl font-bold text-[#d4a017]">{content.stats.value3 || '6'}</span>
                <p className="text-gray-400 text-[10px] sm:text-xs md:text-sm mt-1 sm:mt-2 uppercase tracking-wider">
