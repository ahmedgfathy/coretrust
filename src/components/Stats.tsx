        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {stats.map((s, i) => (
            <div key={i} className="text-center">
              <span className="text-5xl md:text-6xl font-bold gold-shimmer" style={{ fontFamily: 'Playfair Display, serif' }}>{s.number}</span>
              <p className="text-gray-500 mt-3 text-sm uppercase tracking-wider">{s.label}</p>
            </div>
          ))}
        </div>
      </div>
